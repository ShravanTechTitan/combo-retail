  ...
  if (!combo || !model) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 text-red-500">
        ❌ Model not found
        <Link to="/" className="text-blue-500 underline mt-4">Go Home</Link>
      </div>
    );
  }
  ...
  return (
    ...
    <Header></Header>
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        {/* Supported Mobiles */}
        <div>
          <h3 className="text-2xl font-semibold mb-4">Supported Mobiles</h3>
          {filteredMobiles.length > 0 ? (
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {filteredMobiles.map((mobile, index) => (
                <div
                  key={index}
                  className="p-4 bg-white dark:bg-gray-800 rounded-2xl shadow hover:shadow-lg transition transform hover:-translate-y-1"
                >
                  {mobile}
                </div>
              ))}
