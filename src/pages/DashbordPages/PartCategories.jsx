...
export default function PartCategoryPage() {
  ...
  // Apply pagination
  useEffect(() => {
    setPagination((prev) => ({ ...prev, total: Partcategories.length }));
    const start = (pagination.page - 1) * pagination.limit;
    const end = start + pagination.limit;
    setFilteredCategories(Partcategories.slice(start, end));
  }, [Partcategories, pagination.page, pagination.limit]);
              ...
              {filteredCategories.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-3 py-4 text-center text-gray-500 dark:text-gray-400">
                    {Partcategories.length === 0 ? "No categories found." : "No categories on this page."}
                  </td>
                </tr>
              ) : (
